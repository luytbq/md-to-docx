...
export function tcell(text, { width, bold = false, italic = false, color, fill, size, borders: b, align = 'left' } = {}, cfg) {
  const rOpts = { bold, italics: italic, size: (size ?? cfg.table.rowSize) * 2, font: cfg.body.font };
  if (color) rOpts.color = color;
  const cellOpts = {
    children: [new Paragraph({ alignment: atype(align), spacing: { after: 0 }, children: makeRuns(text, rOpts, cfg) })],
    margins: pad(cfg),
...
export function mdTable(block, cfg, CW) {
  ...
  const rows = [
    new TableRow({ children: block.headers.map((h, i) => tcell(h, { width: widths[i], bold: cfg.table.headerBold, color: cfg.table.headerColor, fill: cfg.table.headerFill, size: cfg.table.headerSize, borders: BORDERS, align: 'center' }, cfg)) }),
    ...block.rows.map((row, ri) => new TableRow({ children: row.map((c, i) => tcell(c, { width: widths[i], fill: ri % 2 === 0 ? cfg.table.oddFill : cfg.table.evenFill, color: cfg.table.rowColor, size: fontSize, borders: BORDERS, align: block.align[i] || 'left' }, cfg)) })),
  ];
