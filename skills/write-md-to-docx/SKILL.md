...
`@toc` inserts a real Word TOC field at its position (`levels=N` or `N-M`, default `1-3`;
`hyperlink=false` to disable links). Write the TOC title yourself with a `@style` line, **not** a
`#` heading (a heading would list itself). Page numbers can't be pre-computed: Word asks
"update fields?" on open — answer Yes (LibreOffice: Tools → Update → Update All).
...
| `toc` | Unknown/invalid `@toc` option | Use `levels=N`/`N-M` (1-6) or `hyperlink=false` |
